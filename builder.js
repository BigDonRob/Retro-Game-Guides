...
(function () {
  ...
  async function saveZipToDirectory() {
    const m = state.meta;
    if (!m.raId || isNaN(parseInt(m.raId))) { alert('Set a valid RA Game ID before generating.'); return; }
    if (!m.primaryName) { alert('Set a game title before generating.'); return; }
    if (!state.tabs.length) { alert('Add at least one tab before generating.'); return; }
    ...
    // Create a file input element to trigger directory save dialog
    const input = document.createElement('input');
    input.type = 'file';
    input.webkitdirectory = true;
    input.multiple = true;
    
    // For now, just download the file to the default download location
    // User will need to manually move it to the correct directory
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `${raId}_submission.zip`;
    a.click();
    URL.revokeObjectURL(a.href);
    
    alert(`ZIP file saved as ${raId}_submission.zip\n\nTo use in view mode, place this file in:\n${gamePath}/`);
  }
  ...
  async function boot() {
    ...
    document.getElementById('b-save-zip-btn')?.addEventListener('click', saveZipToDirectory);
